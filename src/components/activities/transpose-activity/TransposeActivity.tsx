...
export const TransposeActivity: React.FC = () => {
  ...
  const [selectedAvestaCharacter, setSelectedAvestaCharacter] = useState<Letter | undefined>(
    undefined,
  );
  const [selectedAvestaTranscription, setSelectedAvestaTranscription] = useState<
    Letter | undefined
  >(undefined);
  ...
  const [shuffledCurrentSlice, setShuffledCurrentSlice] = useState<Letter[]>(
    [...slices[currentSliceIndex]].sort(() => Math.random() - 0.5),
  );
  ...
  const handleClickOnAvestaLetter = useCallback(
    (letter: Letter) => {
  ...
  const handleClickOnAvestaTranscription = useCallback(
    (letter: Letter) => {
      handleActivityStartIfNeeded();
  ...
  const isAvestaCharacterSelected = useCallback(
    (letter: Letter) => {
  ...
  const isAvestaTranscriptionSelected = useCallback(
    (letter: Letter) => {
              ...
              <div className="container">
                <div className="row row-cols-3 gx-4 justify-content-evenly align-items-center">
                  {currentSlice.map((letter, i) => (
                    <div
                      key={`${i}`}
                      className={`col m-2 d-flex justify-content-evenly align-items-center h-100 ${
                        isAvestaCharacterSelected(letter) ? 'bg-primary' : ''
                      }`}
              ...
              {isCurrentSliceTransposed ? (
                <div className="container h-100 d-flex flex-column justify-content-center">
                  <p className="card-text text-center">
                    <i className="bi bi-check2-circle"></i> Well done !{' '}
                  </p>
                  <p className="card-text text-center">
                    You have done {percentageDone}% of this activity.
                  </p>
                  <p className="card-text text-center">Would you like to continue ? </p>
                  <button className="btn btn-primary mt-2 me-2" onClick={handleContinueOnNextSlice}>
                    Continue
                  </button>
                </div>
              ) : (
                <div className="container h-100">
                  <div className="row row-cols-3 gx-4 justify-content-evenly align-items-center h-100">
                    {shuffledCurrentSlice.map((letter, i) => (
                      <div
                        key={`${i}`}
                        className={`col text-center fs-2 ${
                          isAvestaTranscriptionSelected(letter) ? 'bg-primary' : ''
                        }`}
        ...
        <div className="card bg-dark text-light">
          <div className="card-body d-flex align-items-center flex-column ">
            <p className="card-text flex-fill">
              <i className="bi bi-lightbulb"></i> Letters have been shuffled ! Try to recover the
              pairs ! Select an Avestan character, then select the corresponding transposition.
            </p>
